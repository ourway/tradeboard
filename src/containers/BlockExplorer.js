  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        accessor: "hash",
        Cell: ({ value }) => (
          <Link to={`/blockexplorer/block/${value}`}>
            <ColorHash hash={value} />
          </Link>
        ),
        width: 150
      },
      ...
      },
      {
        ...
        accessor: "reward",
        Cell: ({ value, original }) => {
          let totalOut = original.transactions[0].outputs.reduce(
            (acc, o) => acc + o.amount,
            0
          );
      ...
      },
      {
        ...
        Cell: ({ value, original }) => {
          ...
          let fpb = transactions.map(tx => {
            let totalIn = tx.inputs.reduce(
              (a, { amount }) =>
                a + amount.sign * amount.coef * Math.pow(10, amount.exp),
              0
            );
            let totalOut = tx.outputs.reduce(
              (a, { amount }) =>
                a + amount.sign * amount.coef * Math.pow(10, amount.exp),
              0
            );
          ...
          return (fpb.reduce((acc, f) => acc + f, 0) / fpb.length || 0).toFixed(
            6
          );
        }
      },
